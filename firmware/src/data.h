...
struct TamaState {
  ...
  char     promptTool[96];
  char     promptHint[256];
  char     agentLabel[16];   // active agent short name: "cc","codex","claw","hermes"
};
...
static void _applyJson(const char* line, TamaState* out) {
  ...
  // Sound events: {"type":"sound","event":"attention"|"celebrate"|"error"|"mute"}
  const char* dtype = doc["type"];
  if (dtype && strcmp(dtype, "sound") == 0) {
    extern void soundPlay(const char* event);
  ...
  const char* ag = doc["agent"];
  if (ag) { strncpy(out->agentLabel, ag, sizeof(out->agentLabel)-1); out->agentLabel[sizeof(out->agentLabel)-1]=0; }
