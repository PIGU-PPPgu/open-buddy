...
// Called from data.h when bridge sends {"type":"sound","event":"..."}
void soundPlay(const char* event) {
  if (!settings().sound) return;
  if (strcmp(event, "attention") == 0) {
    // Urgent triple chirp — needs your approval
    M5.Beep.tone(1800, 80); delay(100);
    M5.Beep.tone(1800, 80); delay(100);
    M5.Beep.tone(2200, 120);
  } else if (strcmp(event, "celebrate") == 0) {
    // Rising success chime
    M5.Beep.tone(880,  80); delay(90);
    M5.Beep.tone(1100, 80); delay(90);
    M5.Beep.tone(1320, 120);
  } else if (strcmp(event, "error") == 0) {
    // Low descending buzz
    M5.Beep.tone(600, 120); delay(140);
    M5.Beep.tone(400, 200);
  } else if (strcmp(event, "mute") == 0) {
    // Single click to confirm mute toggle (plays regardless of mute state)
